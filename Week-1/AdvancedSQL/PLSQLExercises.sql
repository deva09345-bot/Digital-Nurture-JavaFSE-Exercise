...
-- Database : Oracle (all exercises follow Oracle PL/SQL syntax)
-- ============================================================
...
BEGIN
    ...
    LOOP
        ...
        IF v_age > 60 THEN
            ...
            AND    InterestRate  > 1;
            DBMS_OUTPUT.PUT_LINE('Discount applied for CustomerID: ' || v_customer_id);
        END IF;
    END LOOP;
    CLOSE c_customers;
    COMMIT;
...
BEGIN
    FOR rec IN c_customers LOOP
        IF rec.Balance > 10000 THEN
            ...
            DBMS_OUTPUT.PUT_LINE('VIP set for CustomerID: ' || rec.CustomerID);
        END IF;
    END LOOP;
    COMMIT;
END;
...
BEGIN
    FOR rec IN c_loans LOOP
        DBMS_OUTPUT.PUT_LINE('Reminder: ' || rec.Name || ' - Loan ' || rec.LoanID ||
...
CREATE OR REPLACE FUNCTION CalculateAge(p_dob IN DATE) RETURN NUMBER AS
BEGIN
...
BEGIN
    ...
    VALUES (AuditLog_seq.NEXTVAL, :NEW.TransactionID, SYSDATE,
            'Transaction inserted: Amount=' || :NEW.Amount || ' Type=' || :NEW.TransactionType);
END;
/
...
CREATE OR REPLACE PACKAGE BODY CustomerManagement AS
    ...
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            DBMS_OUTPUT.PUT_LINE('Customer already exists: ' || p_id);
    END;
...
CREATE OR REPLACE PACKAGE EmployeeManagement AS
    ...
    PROCEDURE UpdateEmployee(p_id IN NUMBER, p_salary IN NUMBER, p_position IN VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY EmployeeManagement AS
    ...
    PROCEDURE UpdateEmployee(p_id IN NUMBER, p_salary IN NUMBER, p_position IN VARCHAR2) AS
    BEGIN
        UPDATE Employees SET Salary = p_salary, Position = p_position WHERE EmployeeID = p_id;
        COMMIT;
...
-- EXEC AddNewCustomer(10, 'New Customer', TO_DATE('1995-05-05','YYYY-MM-DD'), 5000);
...
-- EXEC CustomerManagement.AddCustomer(20,'Test','01-JAN-2000',3000);
